def make_bank(balance):
    ...
    def bank(message, amount):
        "*** YOUR CODE HERE ***"
        nonlocal balance
        if (message == 'withdraw') & (balance >= amount):
            balance -= amount 
            return balance 
        elif (message == 'withdraw') & (balance < amount):
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, input_pwd):
        ...
        # incorrect attempts less than 3
        if len(incorrect_pwd) < 3:
            if (input_pwd == password) & (balance >= amount):
                balance -= amount 
                return balance 
            elif (input_pwd == password) & (balance < amount):
                return 'Insufficient funds'
            else:
                incorrect_pwd.append(input_pwd)
